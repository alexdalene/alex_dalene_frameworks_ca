      ...
      <Card className="group flex flex-col justify-between overflow-hidden">
        <CardContent className="relative px-0">
          {product.discountedPrice < product.price && (
            <span className="absolute right-0 top-0 z-10 rounded-bl-lg bg-background px-2 py-1 text-sm font-bold text-primary">
        ...
        <CardFooter className="relative flex-col items-start gap-6">
          {product.rating === 0 ? (
            <span className="invisible h-6 text-sm">No rating</span>
          ) : (
            <div className="flex">
